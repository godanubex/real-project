...
const RegistrationForm = () => {
  ...
  useEffect(() => {
    const url = 'http://159.223.37.95:3001/books';
...
//const textArea = document.getElementById("textAreaExample");
  ...
  const getStore = () => {
    axios.get('http://159.223.37.95:3001/store').then((response) => {
      setStoreList(response.data);
    
    })
  }
  const updateStoreLineName = (id) => {
    axios.put('http://159.223.37.95:3001/updateline', { linename: newLinename, id: id }).then((response) => {
      setStoreList(
        storeList.map((val) => {
          return val.id == id ? {
            id: val.id,
  ...
  const updateStoreName = (id) => {
    axios.put('http://159.223.37.95:3001/update', { name: newName, id: id }).then((response) => {
      setStoreList(
        storeList.map((val) => {
          return val.id == id ? {
            id: val.id,
  ...
  const updateStoreExp = (id) => {
    axios.put('http://159.223.37.95:3001/updateexp', { exp: newExp, id: id }).then((response) => {
      setStoreList(
        storeList.map((val) => {
          return val.id == id ? {
            id: val.id,
  ...
  const updateStorePronumber = (id) => {
    axios.put('http://159.223.37.95:3001/updatepro', { ProNumber: newProNumber, id: id }).then((response) => {
      setStoreList(
        storeList.map((val) => {
          return val.id == id ? {
            id: val.id,
  ...
  const updateStorePrice = (id) => {
    axios.put('http://159.223.37.95:3001/updateprice', { price: newPrice, id: id }).then((response) => {
      setStoreList(
        storeList.map((val) => {
          return val.id == id ? {
            id: val.id,
  ...
  const deleteStore = (id)=>{
    axios.delete(`http://159.223.37.95:3001/delete/${id}`).then((response)=>{
      setStoreList(
  ...
  const submitHandler = (event) => {
    ...
    axios.post('http://159.223.37.95:3001/books', addFormData);
  ...
  return (
    <div className={styles.justifyContentAround}>
      ...
      action="http://159.223.37.95:3001/books"
